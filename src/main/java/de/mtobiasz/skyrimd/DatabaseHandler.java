...
public class DatabaseHandler {
    ...
    public static ArrayList<String> getLikes(String id) {
        ...
        try (Connection con = DriverManager.getConnection("jdbc:mariadb://localhost:3306/skyrimd", "eisberg", "eisberg"); ResultSet rs = con.createStatement().executeQuery("SELECT * FROM likes WHERE fk_id1 LIKE 'id'")) {
            while (rs.next()) {
    ...
    public static void updateUser(User user) {
        try (Connection con = DriverManager.getConnection("jdbc:mariadb://localhost:3306/skyrimd", "eisberg", "eisberg")) {
            PreparedStatement pstm = con.prepareStatement("UPDATE user SET username = ?, photo = ?,  location = ?, birthday = ?, description = ?, gender = ?, password = ? WHERE id = ?");
